...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        # Enter registered email in reset password form
        ...
        # -> Simulate using the reset link to navigate to reset password page
        await page.goto('http://localhost:3000/reset-password?token=mock-reset-token', timeout=10000)
        await asyncio.sleep(3)
        

        # -> Click on 'Forgot password?' link again to retry navigation to reset password form
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError("Test case failed: Password reset process did not complete successfully as per the test plan. The expected confirmation message 'Password reset successful!' was not found on the page.")
