...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Forgot password?' link to navigate to password reset request page.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Password reset completed successfully!')).to_be_visible(timeout=30000)
        except AssertionError:
            raise AssertionError("Test case failed: Password reset process did not complete successfully as expected. The confirmation message for password reset success was not found on the page.")
